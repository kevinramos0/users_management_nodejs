...
const CheckAuth = async (req, res, next) => {
  try {
    ...
    if (!authorization) {
      return res.status(StatusCode.Unauthenticated).json({ message: 'Unauthenticated' });
    }
    ...
    if (!validToken) {
      return res.status(StatusCode.Unauthenticated).json({ message: 'Token invalid' });
    }
    ...
    if (!existRefreskToken) {
      return res.status(StatusCode.Unauthenticated).json({ message: 'refresh Token invalid' });
    }
    ...
    if (dateToken < DateNow) {
      res.status(StatusCode.Unauthenticated).json({ message: 'Token expired' });
    }
